...
class CL_Grabber():
    ...
    def displayData(self, rcvPkt):
        ...
        if (dataDict["dataType"] == "CL"):
            ...
            self.tool.consoleMessage("Frame num: {}, no.of clusters: {}, time: {}".format(frameNum, numClusters, curTime))
            self.tool.consoleMessage("==========================================================================================================================================");
            self.tool.consoleMessage("    #   Id  lane  region       X(m)       Y(m)       Z(m)      Vx(km/h)      Vy(km/h)      Vz(km/h)     L(m)       W(m)       H(m)   Class")
            ...
            for i in range(0, numClusters):
                ...
                self.tool.consoleMessage("{:5d}{:5d}{:6d}{:8d}{:11.2f}{:11.2f}{:11.2f}{:14.2f}{:14.2f}{:14.2f}{:9.2f}{:11.2f}{:11.2f}{:>8s}"
            ...
            if self.headerPrint == 0:
                self.tool.consoleMessage("")
                self.tool.consoleMessage("================================================================================================================");
                self.tool.consoleMessage("TimeStamp                       TrackID   Trigger    X(m)    Y(m)    Vx(km/h)    Vy(km/h)  Lane  objType    L(m)");
            ...
            self.tool.consoleMessage("[{:s}]\t{:5d}{:>10s}{:8.2f}{:8.2f}{:12.2f}{:12.2f}{:6d}{:>8s}{:9.2f}"
               .format(curTime,trPkt["trackId"],triggertype,trPkt["x"]/10,trPkt["y"]/10,(trPkt["vel_x"] * 3.6)/10,
                       (trPkt["vel_y"] * 3.6)/10,trPkt["laneNumber"],objType, (trPkt["length"]/10)))
            ...
            try:
                with open(self.logPath + '/TR.csv', 'a',newline='') as f_object:
                    ...
                    if self.saveTriggerHeader == 0:
                        writer_object.writerow(["time","numErr","TrackID","Trigger","X(m)","Y(m)","Vx(km/h)","Vy(km/h)","Lane","objType","L(m)"])
